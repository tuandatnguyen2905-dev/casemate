...
function statusFor(timeline?: ProgramTimelineRecord) {
  const status = deadlineStatus(timeline);
  if (status.kind === 'countdown') {
    return { label: 'Open', detail: status.label, tone: 'success' as const };
  }
  if (status.kind === 'closed') {
    return { label: 'Closed', detail: status.closeDateLabel, tone: 'muted' as const };
  }
  return { label: 'TBC', detail: status.openText || undefined, tone: 'warning' as const };
}

function StatusBadge({ timeline }: { timeline?: ProgramTimelineRecord }) {
  const status = statusFor(timeline);
...
function ProgramDetail({ program, onClose }: { program: BrowseProgram; onClose: () => void }) {
  ...
  const windowDisplay = applicationWindowDisplay(timeline);
        ...
        <div className="space-y-4 p-4 md:space-y-5 md:p-8">
                    ...
                    <ul className="mt-2 space-y-2">
                      {category.criteria.map((criterion) => (
                        <li key={criterion.id} className="text-xs leading-5 text-[var(--space-text-secondary)]">
                          <span className="mr-1 text-[var(--space-text-brand)]">•</span>{criterion.levels.dat}
                        </li>
                      ))}
                    </ul>
          ...
          <section className="rounded-2xl border border-[var(--space-border-default)] bg-[var(--space-surface-card)] p-5">
            ...
            {timeline?.rounds?.length ? (
              <div className="mt-3">
                <p className="text-sm font-extrabold text-[var(--space-text-primary)]">{timeline.rounds.length} selection rounds</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {timeline.rounds.map((round, index) => (
                    <span key={`${round.name}-${index}`} className="rounded-full bg-[var(--space-surface-muted)] px-3 py-1.5 text-xs font-semibold text-[var(--space-text-secondary)]">R{index + 1} · {round.name}</span>
                  ))}
                </div>
              </div>
          ...
          {timeline?.programUrl && (
            <a href={timeline.programUrl} target="_blank" rel="noreferrer" className="flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--space-brand-primary)] px-4 py-2.5 text-sm font-semibold text-[var(--space-text-on-primary)] transition hover:bg-[var(--space-brand-primary-700)] md:py-3 md:text-base">
...
export default function ProgramBrowser() {
  ...
  const programs = useMemo<BrowseProgram[]>(() => {
    ...
    for (const row of rows || []) {
      ...
      if (!current || row.version > current.version || (row.version === current.version && row.id > current.id)) newestRows.set(row.program_id, row);
    }
    return PROGRAM_FIT_RUBRIC_DRAFTS.map((draft) => ({
      ...draft,
      row: newestRows.get(draft.program_id),
                  ...
                  <article key={program.program_id} className="group flex min-h-0 flex-col rounded-2xl border border-[var(--space-border-default)] bg-[var(--space-surface-card)] p-4 text-left md:min-h-64 md:p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-[var(--space-brand-primary-200)] hover:bg-[var(--space-surface-card-hover)] hover:shadow-md">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <StatusBadge timeline={program.timeline} />
                        <CompetitiveBadge rate={program.competitive} />
                      </div>
                    </div>
                    <div className="mt-4 flex-1 md:mt-5">
                      ...
                      <p className="mt-2 text-xs font-semibold text-[var(--space-text-muted)]">Deadline: {statusFor(program.timeline).detail || statusFor(program.timeline).label}</p>
