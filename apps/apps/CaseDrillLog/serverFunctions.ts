...
export const CASE_DRILL_HOOK_VERSION = 'case-drill-v14-email-access';
...
` +
  ...
  String.raw`
// Case Pool has its own server-side generation gate. Resolve one
// authenticated email, then apply the same explicit order as Case Drill:
// team, active tester, paid Pro, active founding member. A prior trial or
// trial-ended record cannot override a newer tester grant.
async function resolveCasePoolFeatureAccess() {
  ...
  // 2. Temporary tester access. free_until is an inclusive UTC date.
  ...
  // 3. Active Pro, resolved by authenticated email. sessionId proves that the
  // email belongs to this request; it is not the entitlement lookup key.
...
async function ensureProForGenerate() {
  let access = null;
  try {
    access = await resolveCasePoolFeatureAccess();
  } catch (error) {
    console.error('Case Pool access verification unavailable', error);
    respond(503, {
  ...
  respond(402, {
    error: 'Case Pool is part of Casemate Pro, and this account does not have an active subscription or free-access window. Open Settings → Account & Subscription and select Get Pro Plan. Your fit assessment stays completely free.',
    code: 'subscription_required',
    upgrade: { app_id: 'case-drill-log', app_name: 'Case Pool', deep_link: 'app://case-drill-log' },
  });
  return false;
}
...
export async function callCaseDrillServerFunction(
  action: 'generate' | 'grade' | 'grade_library' | 'walkthrough' | 'reveal',
