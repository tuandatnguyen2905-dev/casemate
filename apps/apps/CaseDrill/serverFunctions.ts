...
export const MICRO_DRILL_HOOK_VERSION = 'micro-drill-v1.6-email-access';
...
` +
  ...
  String.raw`
// Case Drill has its own feature gate because generation must never depend on
// a browser-side badge decision. Resolve one server-verified email, then apply
// the product's explicit priority order: team, active tester, paid Pro, active
// founding member. Every table lookup below is keyed by that email; the paid
// status endpoint performs the email-to-subscription resolution because the
// legacy pro_subscriptions table itself has no email column.
async function resolveCaseDrillFeatureAccess() {
  ...
  const email = await resolveBoundEmail();
  if (!email) return denied;

  // 1. Permanent team access.
  const teamResult = await db.query('team_accounts', { where: { email: email }, limit: 1 });
  if (teamResult.rows && teamResult.rows[0]) {
    return Object.assign({}, denied, { entitled: true, reason: 'team_account', email: email });
  }

  // 2. Temporary tester access. free_until is a DATE and is inclusive, so
  // compare against the end of that UTC calendar day.
  const testerResult = await db.query('temporary_testers', { where: { email: email }, limit: 1 });
  const tester = testerResult.rows && testerResult.rows[0] ? testerResult.rows[0] : null;
  if (tester) {
    const rawFreeUntilMs = Date.parse(tester.free_until || '');
    const freeUntilDate = Number.isFinite(rawFreeUntilMs) ? new Date(rawFreeUntilMs).toISOString().slice(0, 10) : '';
    const testerExpiryMs = freeUntilDate ? Date.parse(freeUntilDate + 'T23:59:59.999Z') : NaN;
    if (Number.isFinite(testerExpiryMs) && nowMs <= testerExpiryMs) {
      return Object.assign({}, denied, {
        entitled: true,
        reason: 'temporary_tester',
        email: email,
        days_remaining: Math.max(0, Math.ceil((testerExpiryMs - nowMs) / 86400000)),
        trial_end_date: freeUntilDate,
      });
    }
  }

  // 3. Active Pro. The authenticated email is sent to the platform resolver;
  // sessionId is proof of ownership, not the lookup key.
  let paidCheckError = null;
  try {
    const paid = await proFetchJson(PRO_API_BASE + '/api/space/' + PRO_SPACE_ID + '/subscription-status?email=' + encodeURIComponent(email) + '&sessionId=' + encodeURIComponent(sessionId));
    if (!paid.ok) throw new Error('Subscription service unavailable (status ' + paid.status + ')');
    const status = String(paid.body.status || '').toLowerCase();
    const subscriptionStatus = String(paid.body.subscriptionStatus || paid.body.subscription_status || '').toLowerCase();
    if (status === 'active' || subscriptionStatus === 'active' || status === 'manual_override' || !!paid.body.manualOverride || !!paid.body.manualSubscriptionOverride) {
      return Object.assign({}, denied, { entitled: true, reason: 'subscription', email: email });
    }
  } catch (error) {
    paidCheckError = error;
  }

  // 4. Active founding-member access, keyed by the same verified email.
  const foundingResult = await db.query('founding_members', { where: { email: email }, limit: 1 });
  const founding = foundingResult.rows && foundingResult.rows[0] ? foundingResult.rows[0] : null;
  if (founding) {
    const foundingExpiryMs = Date.parse(founding.expires_at || '');
    if (Number.isFinite(foundingExpiryMs) && nowMs < foundingExpiryMs) {
      return Object.assign({}, denied, {
        entitled: true,
        reason: 'founding_member',
        email: email,
        founding: { member: true, active: true, email: email, expires_at: new Date(foundingExpiryMs).toISOString(), slots_remaining: 0 },
      });
    }
  }

  // Do not turn a paid-status outage into a paywall for a real subscriber.
  if (paidCheckError) throw paidCheckError;
  return Object.assign({}, denied, { email: email });
}

async function ensureProForGenerate() {
  let access = null;
  try {
    access = await resolveCaseDrillFeatureAccess();
  } catch (error) {
    console.error('Case Drill access verification unavailable', error);
    respond(503, {
  ...
  respond(402, {
    error: 'Case Drill is part of Casemate Pro, and this account does not have an active subscription or free-access window. Open Settings → Account & Subscription and select Get Pro Plan. Your fit assessment stays completely free.',
    code: 'subscription_required',
    upgrade: { app_id: 'case-drill', app_name: 'Case Drill', deep_link: 'app://case-drill' },
  });
  return false;
}
