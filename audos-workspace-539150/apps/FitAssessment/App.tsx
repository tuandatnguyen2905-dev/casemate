...
type FitEventRole = 'open' | 'step' | 'activation' | 'core_action' | 'retention';

interface FitEventOptions {
  completionKey?: string | null;
}

function createFitEventId(): string {
...
export default function FitAssessmentApp() {
  ...
  const emitFitEvent = useCallback(async (
    eventName: string,
    eventRole: FitEventRole,
    properties: Record<string, unknown>,
    options: FitEventOptions = {},
  ) => {
    if (!sessionId) return;
    const eventId = createFitEventId();
    const unresolved = userKey === 'device:anonymous' || userKey.startsWith('device:wses_');
    try {
      await (window as any).__workspaceDb?.from('feature_events').insert({
        ...
        feature_name: 'fit',
        app_id: 'fit-assessment',
        mode: null,
        user_id: unresolved ? null : userKey,
        identity_status: unresolved ? 'identity_unresolved' : 'resolved',
        source_user_key: userKey,
      ...
      console.warn('[Fit Assessment] feature_events write failed:', eventName, error);
    }
  ...
  }, [sessionId, trackEvent, userKey]);

  useEffect(() => {
    if (!sessionId || fitStartedTrackedRef.current) return;
    fitStartedTrackedRef.current = true;
    void emitFitEvent('fit_started', 'open', {});
  }, [sessionId, emitFitEvent]);
  ...
  const parseInput = async ({ file, text }: { file?: File; text?: string }) => {
    ...
    try {
      ...
      void emitFitEvent('fit_cv_uploaded', 'step', {
        source: file ? 'file' : 'text',
        file_type: file ? (file.name.toLowerCase().split('.').pop() || null) : null,
        duration_sec: null,
      });
  ...
  const calculateFunctions = async () => {
    ...
    try {
      ...
      void emitFitEvent('fit_step_completed', 'step', { step: 4, step_name: 'mbti', duration_sec: null });
      void emitFitEvent('fit_function_viewed', 'activation', {
        top_function: data.top_functions?.[0]?.name || null,
        top_score: data.top_functions?.[0]?.score ?? null,
      });
  ...
  const loadPrograms = async () => {
    ...
    try {
      ...
      void emitFitEvent('fit_step_completed', 'step', { step: 5, step_name: 'function_fit', duration_sec: null });
  ...
  const openRoadmap = () => {
    ...
    void emitFitEvent('fit_step_completed', 'step', { step: 6, step_name: 'program_selection', duration_sec: null });
    void emitFitEvent('fit_program_selected', 'core_action', {
      program_id: program.program_id,
      program: program.program,
      company: program.company,
      match_percent: program.program_fit_percent,
    }, { completionKey: `fit:${sessionId || 'unresolved'}:${assessmentSessionId}` });
    void emitFitEvent('fit_roadmap_created', 'retention', {
      program_id: program.program_id,
      program: program.program,
      company: program.company,
    });
      ...
      <div className="mx-auto w-full max-w-5xl px-3 py-4 sm:px-6 sm:py-8">
        ...
        {step === 2 && (
          <section className="rounded-2xl border border-[var(--space-border-default)] bg-[var(--space-surface-card)] p-4 sm:p-8"><p className="text-xs font-bold uppercase tracking-widest text-[var(--space-text-brand)]">Stage 1 · Step 2 of 4</p><h2 className="mt-2 text-lg font-extrabold sm:text-xl">What motivates you?</h2><p className="mt-2 text-sm text-[var(--space-text-secondary)]">Four quick choices help separate functions that can look similar on a CV.</p>
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 sm:mt-7 sm:flex-row sm:items-center sm:justify-between"><button type="button" onClick={() => setStep(1)} className="inline-flex items-center justify-center gap-1 text-xs font-bold text-[var(--space-text-muted)] sm:justify-start"><ArrowLeft className="h-3.5 w-3.5" />Back to CV</button><button type="button" disabled={!motivationComplete} onClick={() => { setStep(3); void emitFitEvent('fit_step_completed', 'step', { step: 2, step_name: 'motivation', duration_sec: null }); }} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--space-brand-primary)] px-5 py-2.5 text-sm font-extrabold text-[var(--space-text-on-primary)] disabled:opacity-45 sm:w-auto sm:py-3">Continue to Work Style <ArrowRight className="h-4 w-4" /></button></div>
          </section>
        )}
        ...
        {step === 3 && (
          <section className="rounded-2xl border border-[var(--space-border-default)] bg-[var(--space-surface-card)] p-4 sm:p-8"><p className="text-xs font-bold uppercase tracking-widest text-[var(--space-text-brand)]">Stage 1 · Step 3 of 4</p><h2 className="mt-2 text-lg font-extrabold sm:text-xl">OCP Work Style</h2><p className="mt-2 text-sm text-[var(--space-text-secondary)]">Rate your ideal work environment from 1 to 5. These nine dimensions are compared with each function’s benchmark vector.</p>
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 sm:mt-7 sm:flex-row sm:items-center sm:justify-between"><button type="button" onClick={() => setStep(2)} className="inline-flex items-center justify-center gap-1 text-xs font-bold text-[var(--space-text-muted)] sm:justify-start"><ArrowLeft className="h-3.5 w-3.5" />Back</button><button type="button" onClick={() => { setStep(4); void emitFitEvent('fit_step_completed', 'step', { step: 3, step_name: 'work_style', duration_sec: null }); }} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--space-brand-primary)] px-5 py-2.5 text-sm font-extrabold text-[var(--space-text-on-primary)] sm:w-auto sm:py-3">Continue to MBTI <ArrowRight className="h-4 w-4" /></button></div>
          </section>
        )}
