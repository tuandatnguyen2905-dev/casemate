...
interface StudyModesProps {
  ...
  onReviewReturned: (cardId: string) => void;
}
...
export default function StudyModes({ slug, mode, onBack, onComplete, onReviewReturned }: StudyModesProps) {
  ...
  const review = (card: LearningCard, correct: boolean) => {
    if (memory[card.id]?.lastSeen) onReviewReturned(card.id);
