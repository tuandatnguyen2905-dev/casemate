...
export function SpaceRuntimeProvider({ 
  ...
  const trackEvent = async (eventType: string, metadata: Record<string, any> = {}): Promise<void> => {
    const logMirrorFailure = async (reason: string, statusCode: number | null = null) => {
      try {
        const db = (window as any).__workspaceDb;
        if (!db?.from) return;
        const checkDate = new Date(Date.now() + 7 * 60 * 60 * 1000).toISOString().slice(0, 10);
        await db.from('tracking_health_log').insert({
          check_date: checkDate,
          feature_name: typeof metadata.feature_name === 'string' ? metadata.feature_name : null,
          check_type: 'mirror_failure',
          status: 'warning',
          opened_count: 0,
          downstream_count: 0,
          detail_json: { event_type: eventType, reason, status_code: statusCode },
          observed_at: new Date().toISOString(),
          source_kind: 'monitor',
          session_id: sessionId || null,
        });
      } catch (logError) {
        console.error('[SpaceRuntime] Could not persist mirror failure:', logError);
      }
    };
    if (!sessionId) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        void logMirrorFailure('http_error', response.status);
        return;
      }
      ...
      void logMirrorFailure(err instanceof Error ? err.message : 'network_error');
    }
  };
