...
export const AUTH_HOOK_VERSION = 'auth-v11-linkedin-oidc';
 ...
 *      https://app.casemateaud.com/auth/callback/facebook
...
/** LinkedIn OpenID Connect client ID. Public by design; the secret remains in
 *  the LINKEDIN_CLIENT_SECRET workspace secret and is used only by the hook. */
export const LINKEDIN_CLIENT_ID = '86eo3is2oo48y6';

// Production OAuth callbacks. Keep these exact values in sync with the provider
// consoles — no flow derives its callback from the current page, so preview or
// localhost origins can never leak into redirect_uri.
export const GOOGLE_OAUTH_REDIRECT_URI = 'https://app.casemateaud.com/auth/callback/google';
export const FACEBOOK_OAUTH_REDIRECT_URI = 'https://app.casemateaud.com/auth/callback/facebook';
export const LINKEDIN_OAUTH_REDIRECT_URI = 'https://app.casemateaud.com/auth/callback/linkedin';

// The authenticated shell uses URL hashes as app deep links. OAuth callbacks
// must land here instead of restoring the public EmailGate at `/` first.
export const SOCIAL_OAUTH_SUCCESS_PATH = '/#fit-assessment';
...
export function isLinkedInLoginConfigured(): boolean {
  return LINKEDIN_CLIENT_ID.trim().length > 0;
}
...
export interface AuthServiceStatus {
  ...
  facebookAuth: boolean;
  /** LinkedIn client credentials are stored and accepted by its token endpoint. */
  linkedinAuth: boolean;
}

const STATUS_CACHE_KEY = 'casemate_auth_status_v3';
...
function readCachedStatus(): AuthServiceStatus | null {
  try {
    ...
    return {
      ...
      linkedinAuth: !!parsed.linkedinAuth,
    };
  } catch {
    return null;
  }
  ...
  statusPromise = (async () => {
    ...
    const status: AuthServiceStatus = result.success
      ? {
          ...
          linkedinAuth: !!result.linkedinAuth,
        }
      : { available: false, passwordAuth: false, smsAuth: false, googleAuth: false, facebookAuth: false, linkedinAuth: false };
    if (result.success) writeCachedStatus(status);
    return status;
  })().catch(() => {
    statusPromise = null;
    return { available: false, passwordAuth: false, smsAuth: false, googleAuth: false, facebookAuth: false, linkedinAuth: false };
  });
...
function oauthRedirectUri(provider: 'google' | 'facebook' | 'linkedin'): string {
  if (provider === 'google') return GOOGLE_OAUTH_REDIRECT_URI;
  if (provider === 'facebook') return FACEBOOK_OAUTH_REDIRECT_URI;
  return LINKEDIN_OAUTH_REDIRECT_URI;
}
...
export async function beginSocialOAuth(
  provider: 'google' | 'facebook' | 'linkedin',
  returnTo = new URL(SOCIAL_OAUTH_SUCCESS_PATH, window.location.origin).toString(),
): Promise<void> {
  ...
  if (!configured) {
    ...
    throw new Error(`${label} sign-in is not activated yet.`);
  }

  const state = randomUrlToken(24);
  const codeVerifier = provider === 'google' ? randomUrlToken(48) : undefined;
  const attempt: StoredOAuthAttempt = { provider, state, returnTo, codeVerifier };
 ...
 * workspace session. */
export async function completeSocialOAuthCallback(provider: 'google' | 'facebook' | 'linkedin'): Promise<{
  result: AuthActionResult;
  returnTo: string;
}> {
  const fallbackReturnTo = new URL(SOCIAL_OAUTH_SUCCESS_PATH, window.location.origin).toString();
  let attempt: StoredOAuthAttempt | null = null;
  try {
    attempt = JSON.parse(sessionStorage.getItem(OAUTH_ATTEMPT_KEY) || 'null');
  } catch {
    attempt = null;
  }
  ...
  if (providerError) {
    return {
      result: { success: false, code: 'social_cancelled', error: providerError },
      returnTo: attempt?.returnTo || fallbackReturnTo,
    };
  }
  const code = params.get('code') || '';
  const state = params.get('state') || '';
  if (!attempt || attempt.provider !== provider || !state || state !== attempt.state) {
    return {
  ...
  return { result, returnTo: attempt.returnTo };
}
...
const AUTH_HOOK_DESCRIPTION =
  'Casemate sign-in service (auth-v11): email sign-in is OTP-ONLY and handled by the platform ' +
  ...
  'TWILIO_BASIC_AUTH, host verify.twilio.com), and SERVER-VERIFIED Google/Facebook/LinkedIn sign-in. ' +
  'LinkedIn is callback-only at /auth/callback/linkedin, exchanges its code with LINKEDIN_CLIENT_SECRET, ' +
  'requests openid profile email, and maps verified userinfo name/email/picture into the account/session. ' +
  'Google token audiences and Facebook app IDs are verified before any session exists. ' +
...
// ============================================================================
// Casemate multi-method auth service (casemate-auth-v1 / auth-v11-linkedin-oidc).
...
//                              facebookAuth, linkedinAuth; passwordAuth is always false —
...
//   FACEBOOK_APP_SECRET        (allow-listed host: graph.facebook.com) — this
//                              hook is the ONLY place it is ever used.
...
var LINKEDIN_CLIENT_ID = '__LINKEDIN_CLIENT_ID__';
...
var FACEBOOK_GRAPH_BASE = 'https://graph.facebook.com/v21.0';
...
var FACEBOOK_REDIRECT_URI = 'https://app.casemateaud.com/auth/callback/facebook';
var LINKEDIN_REDIRECT_URI = 'https://app.casemateaud.com/auth/callback/linkedin';
var FACEBOOK_UNCONFIGURED = 'Facebook sign-in is not activated yet. Please use another sign-in method for now.';
var LINKEDIN_UNCONFIGURED = 'LinkedIn sign-in is not activated yet. Please use another sign-in method for now.';
...
async function findAccount(digest) {
  var result = await db.query(AUTH_TABLE, { where: { identifier_digest: digest }, limit: 1 });
...
async function registerPlatformSession(email, extras) {
  ...
  payload.metadata = {
    authMethod: (extras && extras.method) || 'password',
    displayName: (extras && extras.name) || undefined,
    avatarUrl: (extras && extras.avatarUrl) || undefined,
  };
...
async function verifyGoogleIdentity(payload) {
  ...
  var info = null;
  if (idToken) {
    var idResult = await authFetchJson(GOOGLE_TOKENINFO_URL + '?id_token=' + encodeURIComponent(idToken), { method: 'GET' });
    if (idResult.ok && idResult.body && (idResult.body.aud || idResult.body.azp)) info = idResult.body;
  }
  if (!info && accessToken) {
    var atResult = await authFetchJson(GOOGLE_TOKENINFO_URL + '?access_token=' + encodeURIComponent(accessToken), { method: 'GET' });
...
// Every Facebook call that needs provider credentials goes through the secrets
// proxy as an app access token ("APP_ID|APP_SECRET"). Neither workspace secret
...
async function facebookAppRequest(url, query) {
  ...
  params.access_token = '{{secrets.FACEBOOK_APP_ID}}|{{secrets.FACEBOOK_APP_SECRET}}';
...
async function facebookConfigured() {
  try {
    var result = await platform.secretsProxy({
      ...
      query: {
        client_id: '{{secrets.FACEBOOK_APP_ID}}',
        client_secret: '{{secrets.FACEBOOK_APP_SECRET}}',
        grant_type: 'client_credentials',
      },
    });
    return !!result && result.status === 200 && !!(result.body && result.body.access_token);
  } catch (error) {
    return false;
  }
}
...
async function linkedinConfigured() {
  try {
    var result = await platform.secretsProxy({
      method: 'POST',
    ...
    var errorCode = result.body && result.body.error ? String(result.body.error) : '';
...
async function verifyLinkedInIdentity(payload) {
  var accessToken = String((payload && payload.accessToken) || '');
  if (!accessToken) {
    return socialFailure('linkedin_no_token', 'LinkedIn did not return a sign-in token. Please try again.');
  }
  var profile;
  try {
    profile = await authFetchJson(LINKEDIN_USERINFO_URL, {
      method: 'GET',
      headers: { Authorization: 'Bearer ' + accessToken },
    });
  } catch (error) {
    return socialFailure('linkedin_profile_unavailable', 'LinkedIn could not confirm that sign-in. Please try again.');
  }
  if (!profile.ok || !profile.body || !profile.body.sub) {
    return socialFailure('linkedin_token_invalid', 'LinkedIn could not confirm that sign-in. Please try again.');
  }
  var email = normEmail(profile.body.email);
  if (!email) {
    return socialFailure('linkedin_no_email', 'LinkedIn did not share an email address for this account.');
  }
  if (!isTruthyFlag(profile.body.email_verified)) {
    return socialFailure('linkedin_email_unverified', 'LinkedIn has not verified the email on that account. Please use another sign-in method.');
  }
  var name = String(profile.body.name || '').trim();
  var picture = String(profile.body.picture || '').trim();
  var avatarUrl = picture.indexOf('https://') === 0 ? picture : '';
  return {
    ok: true,
    email: email,
    name: name,
    avatarUrl: avatarUrl,
    providerUserId: String(profile.body.sub),
  };
}
...
async function finishVerifiedSocialSignin(provider, verified) {
  ...
  var row = await findAccount(digest);
  var registration = await registerPlatformSession(email, {
  ...
  var now = new Date().toISOString();
  if (row) {
    await db.update(AUTH_TABLE, { id: row.id }, {
      display_name: verified.name || row.display_name || null,
      avatar_url: verified.avatarUrl || row.avatar_url || null,
      failed_attempts: 0,
      locked_until: null,
      last_login_at: now,
    });
...
async function handleOAuthCallback() {
  ...
  var expectedRedirect = provider === 'google'
    ? GOOGLE_REDIRECT_URI
    : provider === 'facebook'
  ...
  try {
    if (provider === 'google') {
      exchange = await platform.secretsProxy({
        method: 'POST',
    ...
    return fail(
      ...
      (provider === 'google' ? 'Google' : provider === 'facebook' ? 'Facebook' : 'LinkedIn') + ' sign-in is temporarily unavailable. Please try again or use email.',
    );
  }
  if (!exchange || exchange.ok !== true) {
    var proxyCode = String(exchange && exchange.code || 'no_result');
    console.error('[casemate-auth] oauth credential proxy failed: ' + proxyCode);
    if (provider === 'facebook' && (proxyCode === 'unknown_secret' || proxyCode === 'no_allowed_hosts' || proxyCode === 'host_not_allowed')) {
      return fail('facebook_not_configured', FACEBOOK_UNCONFIGURED);
    }
    if (provider === 'linkedin' && (proxyCode === 'unknown_secret' || proxyCode === 'no_allowed_hosts' || proxyCode === 'host_not_allowed')) {
      return fail('linkedin_not_configured', LINKEDIN_UNCONFIGURED);
    }
    return fail(
      'oauth_configuration_unavailable',
      (provider === 'google' ? 'Google' : provider === 'facebook' ? 'Facebook' : 'LinkedIn') + ' sign-in is not configured on the server. Please use email for now.',
    );
  }
  if (exchange.status >= 400 || !exchange.body || !exchange.body.access_token) {
    var providerMessage = exchange.body && exchange.body.error_description
      ? String(exchange.body.error_description)
      : '';
    console.error('[casemate-auth] oauth exchange rejected (' + exchange.status + '): ' + providerMessage);
    return fail(
      'oauth_exchange_failed',
      (provider === 'google' ? 'Google' : provider === 'facebook' ? 'Facebook' : 'LinkedIn') + ' could not verify the sign-in code. Please start sign-in again.',
    );
  }
  ...
  var verified = provider === 'google'
    ...
    : provider === 'facebook'
      ...
      : await verifyLinkedInIdentity({ accessToken: exchange.body.access_token });
  return await finishVerifiedSocialSignin(provider, verified);
}
...
async function authMain() {
  if (action === 'status') {
    ...
    var linkedin = LINKEDIN_CLIENT_ID.length > 0 && await linkedinConfigured();
    return ok({
      ...
      passwordAuth: false,
      smsAuth: sms,
      googleAuth: GOOGLE_CLIENT_ID.length > 0,
  ...
  // LinkedIn is intentionally callback-only: accepting an arbitrary bearer
  // token here would not prove that it was issued to Casemate's OAuth client.
  if (action === 'oauth_callback') return await handleOAuthCallback();
...
export function buildAuthHookCode(pepper: string): string {
  return AUTH_HOOK_TEMPLATE.replace("'__PEPPER__'", "'" + pepper + "'")
    ...
    .replace("'__LINKEDIN_CLIENT_ID__'", "'" + LINKEDIN_CLIENT_ID + "'");
}
