...
export interface PreparationRecommendation {
  id: string;
  category: 'Case practice' | 'Domain knowledge' | 'Aptitude preparation' | 'Soft skills' | 'CV & interview';
  gap: string;
  reason: string;
  recommendedAction: string;
  priority: 'Critical' | 'High' | 'Medium' | 'Optional';
  casemateFunction: 'Case Drill' | 'Case Pool' | 'Domain Knowledge' | 'Aptitude Test' | 'Update Profile / CV' | 'My Roadmap';
  order: number;
}
...
export interface SavedRoadmap {
  ...
  active_target_status: string | null;
  fit_result_reference: string | null;
  target_fit_json: Record<string, unknown> | null;
  preparation_recommendations: PreparationRecommendation[];
  roadmap_task_status: Record<string, string>;
...
function currentSpaceId(): string {
  ...
  return (w && (w.__APP_ID__ || w.__SPACE_ID__)) || WORKSPACE_ID;
}
...
function normalizeRow(row: any): SavedRoadmap | null {
  ...
  return {
    ...
    target_program_id: row.target_program_id ? String(row.target_program_id) : null,
    target_function: row.target_function ? String(row.target_function) : null,
    recruitment_cycle: row.recruitment_cycle ? String(row.recruitment_cycle) : null,
    active_target_status: row.active_target_status ? String(row.active_target_status) : null,
    fit_result_reference: row.fit_result_reference ? String(row.fit_result_reference) : null,
    target_fit_json: parseBaselineJson(row.target_fit_json) as Record<string, unknown> | null,
    preparation_recommendations: (parseBaselineJson(row.preparation_recommendations) as unknown as PreparationRecommendation[]) || [],
    roadmap_task_status: (parseBaselineJson(row.roadmap_task_status) as Record<string, string>) || {},
...
export interface GenerateRoadmapInput {
  ...
  recruitmentCycle?: string | null;
  activeTargetStatus?: string | null;
  fitResultReference?: string | null;
  targetFit?: Record<string, unknown> | null;
  preparationRecommendations?: PreparationRecommendation[];
  roadmapTaskStatus?: Record<string, string>;
...
export async function generateAndSaveRoadmap(input: GenerateRoadmapInput): Promise<SavedRoadmap> {
  const db = workspaceDb();
  const { userKey, email } = resolveRoadmapIdentity(input.sessionId);
  const now = new Date();

  const [baseline, windowInfo] = await Promise.all([
    loadRoadmapBaseline(input.sessionId),
  ...
  const record = {
    ...
    target_program_id: input.targetProgramId || null,
    target_function: input.targetFunction || null,
    recruitment_cycle: input.recruitmentCycle || null,
    active_target_status: input.activeTargetStatus || null,
    fit_result_reference: input.fitResultReference || null,
    target_fit_json: input.targetFit || null,
    preparation_recommendations: input.preparationRecommendations || [],
    roadmap_task_status: input.roadmapTaskStatus || {},
  ...
  const saved = await loadLatestRoadmap(input.sessionId);
